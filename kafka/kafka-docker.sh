...
: ${KAFKA_KAFKA_METRICS_REPORTERS:=com.airbnb.kafka.KafkaStatsdMetricsReporter}
: ${KAFKA_EXTERNAL_KAFKA_STATSD_HOST:=$STATSD_PORT_8125_UDP_ADDR}
...
export KAFKA_KAFKA_METRICS_REPORTERS
export KAFKA_EXTERNAL_KAFKA_STATSD_HOST

echo '# Generated by kafka-docker.sh' > $CONFLUENT_HOME/etc/kafka/server.properties

for var in $(env | sort | grep '^KAFKA_'); do
    key=$(echo $var | sed -r 's/KAFKA_(.*)=.*/\1/g' | tr A-Z a-z | tr _ .)
    value=$(echo $var | sed -r 's/.*=(.*)/\1/g')
    echo "${key}=${value}" >> $CONFLUENT_HOME/etc/kafka/server.properties
done

export KAFKA_LOG4J_OPTS="-Dlog4j.configuration=file:$CONFLUENT_HOME/etc/kafka/log4j.properties"

exec kafka-server-start $CONFLUENT_HOME/etc/kafka/server.properties
